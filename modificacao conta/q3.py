...
class ContaEspecial(Conta):
    def __init__(self,id_conta,id_cliente,saldo,limite):
        ...
        self.limite = limite
        self.limite_total = limite

    def saque(self,valor):
        if valor <= (self.saldo+self.limite):
            # diminuindo saldo e limite
            self.saldo -= valor
            # verificando se o saldo ficou negativo
            if self.saldo < 0:
    ...
    def depositar(self, valor):
        faltando = self.limite_total - self.limite
        resto = valor - faltando
        self.limite += faltando
        self.saldo += resto
